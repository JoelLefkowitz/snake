...
bool init_draw(SDL_Surface **surface) {
  *surface = SDL_CreateRGBSurface(0, init_width, init_height, 32, 0, 0, 0, 0);
...
void paint_texture(SDL_Surface *surface, SDL_Renderer *renderer,
...
void draw_rect_at_position(SDL_Surface *surface, Position pos, RGB rgb) {
  ...
  SDL_FillRect(surface, &rect,
               SDL_MapRGB(surface->format, static_cast<Uint8>(rgb.r),
                          static_cast<Uint8>(rgb.g),
                          static_cast<Uint8>(rgb.b)));
}
...
void close_draw(SDL_Surface *surface) { SDL_FreeSurface(surface); }